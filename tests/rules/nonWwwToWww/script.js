...
export default function() {
    group("wyrihaximus.net", function() {
        let res = http.get("http://wyrihaximus.net:1337/");
        let result = check(res, {
            "is status 301": (r) => r.status === 301,
            "location header is set": (r) => r.headers.Location !== undefined,
            "location header is set with correct value": (r) => r.headers.Location === "http://www.wyrihaximus.net:1337/",
        });
        errorRate.add(!result);
    });
    group("bbc.co.uk", function() {
        let res = http.get("http://bbc.co.uk:1337/");
        let result = check(res, {
            "is status 301": (r) => r.status === 301,
            "location header is set": (r) => r.headers.Location !== undefined,
            "location header is set with correct value": (r) => r.headers.Location === "http://www.bbc.co.uk:1337/",
        });
        errorRate.add(!result);
    });
